...
- Why: 왜 이런 일을 하는 건지, 왜 이런 식의 멘토링 진행 방식을 하는 건지
...
- 저는 여러분들이 하려는 경험을 함께 공유하고 제가 여태까지 경험하지 못한 부분을 더 경험하게 됩니다.
- 그러면 나중에 저와 함께 하는 다른 후배들에게 소프트웨어에 대한 이해를 더 잘 이해시켜 줄 수 있는 값진 경험을 얻게 되겠죠.
...
주제 및 목표가 구체화 되고 문서가 나오면 목표를 달성하기 위해 요구사항 분석, 설계, 구현을 필요한대로 진행하는 걸 권장합니다. 특별한 문서 템플릿이나 작성 규칙은 없습니다. 정해진 규칙이 없다고 해서 어렵게 생각하지 마시고 필요하다고 생각되는 부분을 저와 함께 얘기하면서 진행하시면 되고, 많이 어려우신 분들은 제가 적절한 힌트나 가이드 등을 드립니다.
...
저 외에 다른 멘토분들과 함께 다른 목표로 진행한다면 상호평가를 통해 가능한 평가 방법이지만 저의 멘토링 과정은 저 외에 다른 멘토 분들하고 함께 진행하지 않기 때문에 공정성이 있다고 보기는 힘듭니다. 다만, 처음부터 공정성을 부여하지 않으면 역량 평가를 하지 않으면 된다는 [대우](https://ko.wikipedia.org/wiki/%EB%8C%80%EC%9A%B0_(%EB%85%BC%EB%A6%AC%ED%95%99))역시 참인 명제이므로 제가 여러분의 역량을 평가해 드리지 않고 있습니다. 사실 제가 평가해 드리지 않아도 저와 함께 하면서 시간을 보내면 여러분 스스로 평가가 가능합니다.
...
- 학교 과제나 회사 실무 문제를 저한테 시켜서 빨리 해결하고 싶으신 분
  ...
  - 본인의 역량을 더 키워 천천히 해결해 나가세요.
- 사람을 고용해서 해야 할 일을 절 시켜서 하시려는 분
  - 빨리 사람을 구해서 정당하게 댓가를 지불하고 일을 시키세요.
  - 좋은 결과를 얻기를 희망합니다.
...
- 헬스 관리 앱 개발 진행 예정, 사전 React 앱 개발을 위한 훈련
- 2018-12 ~ 현재
- 1주에 한번씩 목요일 23:00 온라인 미팅
  - [헬스 관리 앱, 미팅 기록 정리](https://github.com/WonjeongPark/WhatTheHealth)
  - [ReactJS 훈련, 앱 따라 만들기 강의](https://github.com/WonjeongPark/revents)
- 개발툴: Visual Studio Code
- 프레임워크: React.js 16.3.0, Node.js
- 언어: Javascript
